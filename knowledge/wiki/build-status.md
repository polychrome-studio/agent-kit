...
"Mode is the primitive": one classification per turn drives model + voice + source-chip visibility. `src-tauri/src/router.rs` — `Mode { Quick, Companion, Research, Deep }` → `haiku-4.5 / sonnet-4.6 / sonnet-4.6 / opus-4.8`. **Opus is reserved to `Deep`**, reached only by explicit escalation ("go deep / think hard / be thorough / use opus") — research defaults to Sonnet now that the agent tools do the heavy lifting (2026-06-02, per [[prefer-cheaper-models]] memory). **Hybrid classifier:** instant heuristics for obvious cases (hot path stays snappy), a cheap `claude-3.5-haiku` tie-break only for long unsignalled queries (it never returns Deep — Opus is heuristic/explicit-only). `chat` emits a `Meta { mode, model }` event → both surfaces show a "✦ sonnet 4.6 · companion" label; `Sources` chips emit in research + deep. Full rationale: [[journal/2026-05-31-m4-task-routing]].
